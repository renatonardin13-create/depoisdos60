...
export const ProductBundleMockup: React.FC<ProductBundleMockupProps> = ({ 
  ...
  const handleResetImage = (e: React.MouseEvent) => {
    ...
    setCustomImage(null);
    try {
      localStorage.removeItem('hero_box_mockup_image_v2');
    } catch {
      // ignore
    }
  };
  ...
  return (
    ...
    >
      {/* Hidden file input for uploading the exact ChatGPT Image 6 de set. de 2026, 14_01_37.png */}
      <input 
        ref={fileInputRef}
        type="file" 
        accept="image/*" 
      ...
      {customImage ? (
        ...
        >
          {/* Subtle image upload bar */}
          <div 
            onClick={(e) => {
              e.stopPropagation();
              fileInputRef.current?.click();
            }}
            className={`w-full max-w-[440px] mb-3 px-3 py-1.5 rounded-xl border border-dashed transition-all cursor-pointer flex items-center justify-between gap-2 shadow-sm ${
              isDragging 
                ? 'bg-emerald-500/20 border-emerald-400 text-white scale-102' 
                : 'bg-emerald-950/70 hover:bg-emerald-950/90 border-emerald-500/30 text-emerald-200'
            }`}
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="text-[11px] font-medium text-emerald-100">
                Usar arquivo PNG original da Imagem
              </span>
            </div>
            <span className="px-2 py-0.5 rounded bg-amber-400/90 text-amber-950 text-[10px] font-black uppercase tracking-wider shrink-0">
              Carregar PNG
            </span>
          </div>
